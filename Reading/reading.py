...
#added the os environmental variables into read_file
...
if __name__ == "__main__":
    #initialize a list to hold all sample names
    all_samples = []
    #iterate through the samples dictionary and extend the list with each sample list
    for sample_list in samples.values():
        all_samples.extend(sample_list['list'])

    #created a dictionary to map sample names to their file paths
    #including differentiating between data and Monte Carlo samples
    info_library = {sample: "Data/" if 'data' in sample else f"MC/mc_{infofile.infos[sample]['DSID']}." for sample in all_samples}
    
    #record process starting time
    start = time.time()

    #retrieve the environment variables set for each worker: the sample name, the range of data they process, and the worker's specific ID
    sample, worker_beginning, worker_end, worker_id = os.getenv('SAMPLE'), int(os.getenv('WORKER_BEGINNING')), int(os.getenv('WORKER_END')), int(os.getenv('WORKER_ID'))
    #here I have constructed the file path for the current sample to be processed
    capture_file = os.path.join(tuple_path, info_library[sample] + sample + ".4lep.root")

    #processing the data for the given sample and range using the read_file function
    reading_file = read_file(capture_file, sample, worker_beginning, worker_end, worker_id)
    #defining the file path for the output, which has to be  stored in parquet format to handle awkward array types
    writing_file = f"/mydir/process_info/reading_{sample}-{worker_beginning}-{worker_end}.awkd"
    #saving the processed data to a parquet file for plotting and analysis in the other containers
    ak.to_parquet(reading_file, writing_file)

    #recording the overall timetaken for this main reading process
